...
public interface FinanceRepo extends JpaRepository<UserEntity,String> {
    ...
    @Query(value = "select categoryid ,category_name ,category_type  from categories c  where userid = userid;",nativeQuery = true)
    List<Object[]>  categoryid(@Param("userid") String userid);

    @Query(value = "select mapid ,categoryid ,categorytype ,transactiontype  from categorymapping c  where categoryid = :categoryid and userid = :userid",nativeQuery = true)
    List<Object[]>  mappingid(String categoryid, String userid);
